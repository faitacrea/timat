...
{/* Responsive CSS */}
<style>{`.lp-nav-btns{display:flex;gap:8px;align-items:center} .lp-nav-full{display:flex;gap:8px;align-items:center} .lp-nav-mobile{display:none} .lp-hero-stats{display:flex;gap:32px;flex-wrap:wrap;justify-content:center} .lp-demo-grid{display:grid;grid-template-columns:200px 1fr;gap:24px;align-items:start} .lp-demo-tabs{display:flex;flex-direction:column;gap:6px} .lp-transfo-row{display:grid;grid-template-columns:40px 1fr 1fr 1fr;gap:20px;align-items:center} .lp-tarifs-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start} .lp-logo{font-size:26px;font-weight:700;display:flex;align-items:center;gap:8px;letter-spacing:-.5px} .lp-logo-icon{width:32px;height:32px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px} .lp-hero-ctas{display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-bottom:28px} .lp-hero-ctas button{white-space:nowrap} .lp-hero{padding:0 24px 80px;position:relative;overflow:hidden} .lp-section{padding:72px 24px} .lp-guarantees{display:flex;gap:20px;justify-content:center;flex-wrap:wrap;text-align:center;margin-top:24px;font-size:13px} @media(max-width:768px){ .lp-nav-full{display:none!important} .lp-nav-mobile{display:flex!important;gap:6px;align-items:center} .lp-hero-stats{gap:16px} .lp-hero-stats>div{min-width:60px} .lp-demo-grid{grid-template-columns:1fr!important;gap:16px} .lp-demo-tabs{flex-direction:row;flex-wrap:wrap;gap:4px} .lp-demo-tabs button{padding:8px 12px!important;font-size:12px!important;flex:0 0 auto} .lp-transfo-row{grid-template-columns:1fr!important;gap:8px;padding:14px!important} .lp-transfo-row>div:first-child{display:none} .lp-tarifs-grid{grid-template-columns:1fr!important;gap:16px} .lp-logo{font-size:22px} .lp-logo-icon{width:28px;height:28px;font-size:15px} .lp-section{padding:48px 16px} .lp-hero{padding:0 16px 50px} .lp-hero-ctas{flex-direction:column;align-items:center;gap:10px} .lp-hero-ctas button{width:100%;max-width:320px;text-align:center} .lp-guarantees{flex-direction:column;gap:8px;font-size:12px} } @media(max-width:480px){ .lp-hero-stats{gap:10px} .lp-hero-stats>div{min-width:50px} .lp-demo-tabs button{padding:6px 8px!important;font-size:11px!important} .lp-section{padding:40px 12px} .lp-hero{padding:0 12px 40px} .lp-logo{font-size:20px} .lp-logo-icon{width:24px;height:24px;font-size:13px} }`}</style>
...
‘<div class="sig">’,
‘<div class="sig-box">Fait à ************<br/>Le ‘+new Date().toLocaleDateString(‘fr-FR’)+’<br/><br/>Signature :</div>’,
’<div class="sig-box">Remis au parent le :<br/>************<br/><br/>Signature parent :</div></div>’,
...
const removePain=(idx)=>setCfg(c=>({…c,painPoints:(c.painPoints||[]).filter((*,i)=>i!==idx)}));
const addTesti=()=>setCfg(c=>({…c,testimonials:[…(c.testimonials||[]),{nom:“Nouveau”,ville:“Ville”,avant:“Avant…”,apres:“Après…”}]}));
const removeTesti=(idx)=>setCfg(c=>({…c,testimonials:(c.testimonials||[]).filter((*,i)=>i!==idx)}));
// Free/Pro/Guarantees
...
const removeFreeItem=(idx)=>setCfg(c=>({…c,freeItems:(c.freeItems||[]).filter((*,i)=>i!==idx)}));
...
const removeProItem=(idx)=>setCfg(c=>({…c,proItems:(c.proItems||[]).filter((*,i)=>i!==idx)}));
