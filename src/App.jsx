...
nt)} :#fff;f
    ...
    @media(max-width:768px){.toast{bottom:80px;font-size:12px;padding:10px 16px;border
...
//
const age=(d)=>{const n=new Date(d),t=new Date(),m=(t.getFullYear()-n.getFullYear())*1
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {bilansRecus.map(b=><div key={b.id}>
        <div onClick={()=>setDocOuvert(docOuvert===b.id?null:b.id)}
          ...
          <div>
<div style={{fontWeight:700,fontSize:13,color:"var(--P)"}}> {b.type==="bilan"?" Bilan de journée du "+b.date:" CR Trimestriel
</div>
...
{role==="parent"&&<div style={{fontSize:11,color:"var(--l)"}}>Tapez pour </div>
...
r
 ...
 function RepasChanges({enfants,role,pEId}){
  ...
  useEffect(()=>{
    ...
    supabase.from("changes_couches").select("*").eq("enfant_id",enfant.id).eq("date",T
      .then(({data})=>{if(data)setCh(data.map(r=>({...r,eId:r.enfant_id,h:r.heure,n:r.
    supabase.from("repas").select("*").eq("enfant_id",enfant.id).eq("date",TODAY_STR)
      .then(({data})=>{if(data&&data[0])setRp([{...data[0],eId:data[0].enfant_id,q:dat
  },[enfant?.id,isDemo]);
  const echs=ch.filter(c=>(c.eId||c.enfant_id)===enfant?.id&&c.date===TODAY_STR).sort(
  const erp=rp.find(r=>(r.eId||r.enfant_id)===enfant?.id&&r.date===TODAY_STR);
const addCh=async()=>{
if(!nch.h)return;
const newCh={id:"chn"+Date.now(),eId:enfant.id,date:TODAY_STR,h:nch.h.replace(":", setCh(p=>[...p,newCh]);
...
dej:re.dej??erp?.dej,gou:re.gou??erp?.gou,bib:re.bib??erp?.bib,q:re.q??erp?.q??" setRp(p=>ex?p.map(r=>(r.eId||r.enfant_id)===enfant.id&&r.date===TODAY_STR?up:r):[. setRe({});setToast("Repas enregistré ✓");
if(!isDemo&&enfant?.id){
...
s
 ...
 <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
  ...
  {echs.map(c=><div key={c.id}style={{display:"flex",justifyContent:"space-bet
    ...
    <span className="badge"style={{background:c.type==="Propre"?"var(--Sp)":"v
...
'<div class="box"><h2> Assistante maternelle</h2>', '<table><tr><td>Nom</td><td>'+((enfant?.prenomAsmat||"")+" "+(enfant?.nomAsmat||
   nces:[]
sHebdo|
40)*52/
12)};
...
"")).tr
 ...
 '<tr><td>Enfant gardé</td><td>'+(enfant?.prenom||'-')+' '+(enfant?.emoji||'')+'< '<tr><td>Période</td><td>'+mois+'</td></tr></table></div>',
...
'<table><tr><td>Heures mensualisées (contrat)</td><td>'+hMens+' h</td></tr>', '<tr><td>Heures réellement effectuées</td><td>'+h.real+' h</td></tr>', '<tr><td>Heures complémentaires / supplémentaires</td><td>'+Math.max(0,h.real-hM '<tr><td>Jours d\'activité</td><td>'+joursTrav+' jours</td></tr>', '<tr><td>Jours de congés payés pris</td><td>0 jours</td></tr></table></div>', '<div class="box"><h2> Salaire à déclarer</h2>',
    '<table><tr><td>Salaire net horaire</td><td>'+(totalBrut*0.78/h.real).toFixed(4)
...
<div style={{display:"flex",flexDirection:"column",gap:12}}>
  ...
  <div className="card"style={{padding:14}}>
    <div style={{fontWeight:700,fontSize:13,marginBottom:10,color:"var(--b)"}}>
    {isDemoFact?[["Février 2024","Émise","672.40€"],["Janvier 2024","Payée","698
    ...
    :<div style={{fontSize:12,color:"var(--l)",textAlign:"center",padding:"16px
 r",marg
Total b
{totalB
0.78).t
...
{signes[enfant?.id]&&<div style={{background:"var(--Sp)",border:"1px solid var <divstyle={{fontSize:24,marginBottom:4}}> </div>
...
<button className="btn bP"onClick={generer}> Régénérer</button> <button className="btn bG"onClick={()=>navigator.clipboard?.writeText(cr
      </div>
    </div>}
  </div>
</div>
...
{toast&&<Toast msg={toast}onClose={()=>setToast("")}/>} <PageHeadericon=" "title="EspaceDocuments"
  ...
  {/* Formulaire ajout */}
  {showUpload&&<div className="card"style={{padding:18,marginBottom:16,border:"1.5px
<div style={{fontWeight:700,fontSize:13,marginBottom:12,color:"var(--b)"}}> Aj <div className="g2"style={{marginBottom:10}}>
      <div>
...
<button className="btn bG"style={{padding:"6px 10px",fontSize:12}}
              onClick={()=>{if(doc.url)window.open(doc.url);else setToast("Impress
title="Imprimer"> </button>
{role==="asmat"&&<button className="btn bG"style={{padding:"6px 10px",
...
function BulletinSalaire({enfants,role,pEId,user}){
  ...
  const moisDisponibles=useMemo(()=>{
    const debut=contrat.debut?new Date(contrat.debut):new Date(new Date().getFullYear(
    const now=new Date();
    const mois=[];
    const noms=["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Sept
...
<PageHeadericon=" "title="Bulletindesalaire"sub="Bulletinofficielconforme {role==="asmat"&&<div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"
  ...
  <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
    {moisDisponibles.slice(0,6).map(m=><button key={m}onClick={()=>setMoisSel(m)}sty
    ...
    <div style={{borderBottom:"2px solid var(--b)",paddingBottom:14,marginBottom:14}
      <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap
        ...
        <div style={{textAlign:"right",fontSize:11}}>
          ...
          <div style={{color:"var(--m)"}}>{D.parents.find(p=>p.id===enfant?.parentId
          <div style={{color:"var(--l)"}}>N° Pajemploi : à renseigner sur pajemploi.
        </div>
      </div>
...
a
       ...
       <div className="pf"style={{fontSize:20,fontWeight:700,color:"#fff",marginBottom:
      <div style={{fontSize:13,color:"rgba(255,255,255,.75)",lineHeight:1.7,marginBott
...
return <div>
  ...
  {(enfantPhotos.length>0||role==="asmat")&&<div className="card"style={{padding:16,
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",m <div style={{fontWeight:700,fontSize:14,color:"var(--b)"}}> Photos du jour</ {role==="asmat"&&<>
...
e
 ...
 borderColor:catFilt===c?(catColors[c]||"var(--b)"):(catColors[c]+"44"||"var(-- }}>{c==="tous"?" Tout":c}</button>)}
</div>
    <div style={{fontSize:12,color:"var(--l)",marginBottom:14,fontFamily:"'DM Mono',mo
...
function useInstallPWA(){
  ...
  useEffect(()=>{
    if(window.matchMedia('(display-mode: standalone)').matches){setIsInstalled(true);r
...
function InstallButton(){
  ...
  if(isInstalled)return <div style={{fontSize:12,color:"var(--S)",fontWeight:600}}>
  return <>
...
function InstallGuide({isIOS,onClose}){
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",display:"fl
    onClick={e=>e.target===e.currentTarget&&onClose()}>
    <div style={{background:"var(--w)",borderRadius:"20px 20px 0 0",padding:"24px 20px
<div style={{textAlign:"center",marginBottom:20}}> <divstyle={{fontSize:36,marginBottom:8}}> </div>
<div style={{fontWeight:700,fontSize:17,color:"var(--b)",marginBottom:4}}>Inst <div style={{fontSize:13,color:"var(--l)"}}>Ajoutez l'icône sur votre écran d'
      </div>
      {isIOS?<div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20
{[["1"," Appuyez sur Partager","En bas de Safari"],["2"," Faites défiler", <div key={n} style={{display:"flex",gap:12,padding:"10px 14px",background:"v <div style={{width:28,height:28,borderRadius:"50%",background:"var(--T)",c <div><div style={{fontWeight:600,fontSize:13,color:"var(--b)"}}>{t}</div><
          </div>)}
        <div style={{padding:"10px 14px",background:"var(--Bp)",borderRadius:10,fontSi
             Fonctionne uniquement sur <strong>Safari</strong>
        </div>
</div>:<div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20 {[["1","⋮ Appuyez sur les 3 points","En haut à droite de Chrome"],["2","Ajouter
          <div key={n} style={{display:"flex",gap:12,padding:"10px 14px",background:"v
            <div style={{width:28,height:28,borderRadius:"50%",background:"var(--T)",c
            <div><div style={{fontWeight:600,fontSize:13,color:"var(--b)"}}>{t}</div><
          </div>)}
      </div>}
      <button onClick={onClose} style={{width:"100%",background:"var(--T)",color:"#fff
     TiMat
ex",ali
 32px",
aller T
...
function BandeauInstall(){
  ...
  return <>
    <div style={{background:"linear-gradient(135deg,var(--T),var(--S))",padding:"8px 1
<spanstyle={{fontSize:18}}> </span> <div style={{flex:1}}>
        <div style={{fontSize:12,fontWeight:700,color:"#fff"}}>Installer TiMat sur vot
        <div style={{fontSize:10,color:"rgba(255,255,255,.75)"}}>Accès rapide comme un
      </div>
      <button onClick={()=>install(()=>{if(!deferredPrompt)setShowGuide(true);})}
        style={{background:"rgba(255,255,255,.2)",border:"1px solid rgba(255,255,255,.
        Installer
</button>
      <button onClick={()=>setShow(false)} style={{background:"none",border:"none",col
    </div>
    {showGuide&&<InstallGuide isIOS={isIOS} onClose={()=>setShowGuide(false)}/>}
  </>;
}
...
<div className="card"style={{padding:20}}>
  ...
  {user?.role==="asmat"&&<div style={{marginTop:12}}>
    ...
    <div style={{display:"flex",gap:8}}>
      <input className="inp" defaultValue={user?.code_postal||""} id="cp-input"
...
placeho
 ...
 if(!cp)return;
await supabase.from("profiles").update({code_postal:cp}).eq("id",user.id setUser&&setUser(u=>({...u,code_postal:cp}));
// Forcer rechargement page PMI
const dep=cp.slice(0,2);
const pmi={"75":"PMI Paris 75","92":"PMI Hauts-de-Seine 92","93":"PMI Se setToast(" Code postal "+cp+" enregistré — "+pmi+" — Allez dans Outils
      }}>Sauvegarder</button>
    </div>
    {user?.code_postal&&<div style={{fontSize:11,color:"var(--S)",marginTop:4}}>
         Code postal : {user.code_postal} → PMI {{"75":"Paris 75","92":"Hauts-de
    </div>}
  </div>}
</div>
{/* Installation PWA */}
<div className="card"style={{padding:20}}>
<div style={{fontWeight:700,fontSize:14,color:"var(--b)",marginBottom:8}}> I <div style={{fontSize:12,color:"var(--l)",marginBottom:12,lineHeight:1.6}}>
    Ajoutez TiMat sur votre écran d'accueil pour y accéder comme une vraie appli
  </div>
  <InstallButton/>
</div>
<div className="card"style={{padding:20}}>
...
i
   ...
   return <div>
    <div style={{display:"flex",gap:2,marginBottom:14,borderBottom:"1.5px solid var(--
{[{id:"messages",l:"Messages",ic:" "},{id:"bilan",l:"Bilandujour",ic:" "},{i <button key={s.id}onClick={()=>setSousSec(s.id)}style={{
...
}}onClick={()=>toggleVaccin(i)}> <spanstyle={{fontSize:20,flexShrink:0}}>{v.fait?" ":enRetard?" ":proche <div style={{flex:1}}>
...
function DocumentsComplet({enfants,role,pEId,user}){
  ...
  return <div className="fi">
<PageHeadericon=" "title="Documents&Attestations"sub="Tousvosdocumentset <div style={{display:"flex",gap:2,marginBottom:16,borderBottom:"2px solid var(--br {[{id:"documents",l:"Documents",ic:" "},{id:"attestation_pe",l:"Att.PoleEmplo
...
[" Naissance",fmt(sel.enfant.naissance)+" ("+ageEnfant(sel.enfant.naissa
...
setToast("Contrat signé électroniquement ✓ - L'assmat a été notifiée");
  ...
  <divstyle={{fontSize:60,marginBottom:16}}> </div>
  ...
  const generer=()=>{
    ...
    setTimeout(()=>{
      ...
      const htmlRapport='<!DOCTYPE html><html><head><title>Rapport annuel '+annee+' -
        ...
        +'<p><strong>Enfant:</strong> '+(enfant?.prenom||'')+' '+(enfant?.nom||'')+'</
...
p>'
 ...
 +"<tr><td>Crédit d'impôt estimé (50%)</td><td>"+creditImpot+"€</td></tr>" +'</table>'
...
[" ","Récapitulatifmensueldesheures(jan→déc)"],
[" ","Totalsalairenetmensueletannuel"],
...
return <div className="fi">
{toast&&<Toast msg={toast}onClose={()=>setToast("")}/>} <PageHeadericon=" "title="Exportdevosdonnées"
...
<button className="btn bT"style={{width:"100%",justifyContent:"center"}}onCl {exporting?" Génération en cours...":" Exporter mes données"}
          </button>
        </div>
...
{id:"calendrier",l:"Calendrier",ic:" "}, {id:"messagerie",l:"Messagerie",ic:" "},
               ding}>
center"
 ","Cen
rRadius
...
> (--R)",
 t
  ...
  {id:"admin_finances",l:"Facturation&Bilans",ic:" "}, {id:"documents_complet",l:"Documents&Attestations",ic:" "}, {id:"bilans_exports",l:"Bilans&Exports",ic:" "},
]},
outils:{l:"OutilsPro",ic:" ",color:"#FF9F63",subs:[
{id:"inviter_parent",l:"Inviterunparent",ic:" "}, {id:"projet_accueil",l:"Projetd'accueil",ic:" "}, {id:"pmi",l:"PMI",ic:" "}, {id:"faq",l:"Centred'aide",ic:" "}, {id:"support",l:"Support",ic:" "},
]}, };
const GROUPS_P={
...
<div style={{fontSize:13,fontWeight:700,color:"#264653",marginBottom:12}}> J <div style={{background:"#F0FAF4",borderRadius:10,padding:12,marginBottom:8,bo
...
background:valide?"#2A9D8F":"#F4F7FA",color:valide?"#fff":"#264653",transi }}>{valide?" Pointage validé":"Valider le pointage"}</button>
        </div>
...
const [loading, setLoading] = useState(false);
const [consent, setConsent] = useState({politique:false, cgu:false, newsletter:false const consentValide = consent.politique && consent.cgu;
const [demoMsg, setDemoMsg] = useState(""); const[demoMood,setDemoMood]=useState(" ");
...
<div id="demo" className="lp-section" style={{ background: L.section2Bg||"#FDF5F
  <div style={{ maxWidth: 1000, margin: "0 auto" }}>
    ...
    <div style={{ display: "flex", justifyContent: "center", alignItems: "flex-s
      {/* Phone frame — vraie UI de l'app */}
      <div style={{ width: 340, flexShrink: 0, background: "#1a1a2e", borderRadi
        {/* Notch */}
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 8
<div style={{ width: 110, height: 24, background: "#1a1a2e", borderRad
135deg,
r: L.s1
, lineH
(260px,
...
us: 44,
    ...
    <div style={{ width: 48, height: 4, borderRadius: 2, background: "#2
  </div>
</div>
{/* Screen */}
<div style={{ background: "#FDFBF8", borderRadius: 30, overflow: "hidden
  {/* Badge DEMO */}
  <div style={{position:"absolute",top:8,right:8,zIndex:10,background:"r
  {/* TopBar */}
  <div style={{ background: "rgba(255,255,255,.98)", borderBottom: "1px
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <span style={{ fontFamily: "Georgia,serif", fontSize: 16, fontWeig
      <span style={{ fontSize: 8, color: "#aaa", letterSpacing: 1 }}>v3<
    </div>
    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
<div style={{position:"relative"}}> <spanstyle={{fontSize:14}}> </span>
<div style={{position:"absolute",top:-3,right:-3,width:8,height:
</div>
...
<div style={{ width: 24, height: 24, borderRadius: "50%", backgrou
  ...
  {/* Sous-onglets */}
  {demoGroup!=="accueil"&&<div style={{ background: "#fff", borderBottom
{(demoGroup==="enfant"?[
{id:"journal",l:" Journal",ok:true},{id:"pointage",l:" Pointag {id:"sante",l:" Sante",ok:false},{id:"urgence",l:" Urgence",ok
]:demoGroup==="admin"?[
{id:"messagerie",l:" Messages",ok:true},{id:"facturation",l:" {id:"calendrier",l:" Calendrier",ok:false},{id:"documents",l:"
]:[
{id:"projet",l:" Projet",ok:false},{id:"pmi",l:" PMI",ok:false
    ]).map(s=><div key={s.id} onClick={s.ok?()=>setDemoPage(s.id):undefi
      whiteSpace:"nowrap",padding:"4px 10px",borderRadius:8,fontSize:9,f
      background:demoPage===s.id?"linear-gradient(135deg,#C4714A,#D4824A
      color:demoPage===s.id?"#fff":s.ok?"#9B6BAA":"#C0B8C8",opacity:s.ok
}}>{s.l}{!s.ok&&" "}</div>)} </div>}
  {/* Contenu */}
...
"
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,ma
    {[{v:D.enfants.length,l:"Enfants",c:"#C4714A"},{v:demoMsgs.filte
  ...
  {D.enfants.map(e=>{
    ...
    return <div key={e.id} onClick={()=>setDemoArrivee(prev=>({...pr
      <span style={{fontSize:20}}>{e.emoji}</span>
      <div style={{flex:1}}>
        <div style={{fontSize:11,fontWeight:700,color:"#264653"}}>{e
        <div style={{fontSize:9,color:"#9B6BAA"}}>{estPresent?"Arriv
      </div>
  ...
  <div style={{background:"#FFF8F3",borderRadius:10,padding:8,margin
    ...
    <div style={{fontSize:9,color:"#264653",lineHeight:1.9}}>Journal
  </div>
</div>}
{/* JOURNAL */}
{demoPage==="journal"&&<div style={{padding:10}}>
  <div style={{fontSize:12,fontWeight:700,color:"#264653",marginBott
  {D.transmissions.filter(t=>t.eId==="e1").map(t=><div key={t.id} st
    ...
    <div style={{display:"flex",justifyContent:"space-between",align
      ...
      <button onClick={()=>setDemoLiked(v=>!v)} style={{background:"
    </div>
  </div>)}
  <div style={{display:"flex",gap:4,marginBottom:6}}>
{[" "," "," "," "," "," "].map(m=><divkey={m}onClick={( </div>
  <div style={{display:"flex",gap:5}}>
    <input value={demoMsg} onChange={e=>setDemoMsg(e.target.value)}
    <div onClick={()=>{if(demoMsg.trim())setDemoMsg("");}} style={{b
  </div>
</div>}
{/* POINTAGE */}
...
D
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:
    {[{l:"Prevues",v:"174h",c:"#264653"},{l:"Realisees",v:"152h30",c
  ...
  {D.enfants.map(e=>{
    ...
    return <div key={e.id} onClick={()=>setDemoArrivee(prev=>({...pr
      ...
      <div style={{flex:1}}><div style={{fontSize:10,fontWeight:700,
      <div style={{fontSize:9,fontWeight:700,background:estPresent?"
</div>; })}
</div>}
{/* MESSAGERIE */}
{demoPage==="messagerie"&&<div style={{padding:10,display:"flex",fle
  <div style={{fontSize:12,fontWeight:700,color:"#264653",marginBott
  <div style={{display:"flex",gap:5,marginBottom:8,overflowX:"auto"}
    {D.enfants.map(e=>{const unread=demoMsgs.filter(m=>m.eId===e.id&
      <span style={{fontSize:13}}>{e.emoji}</span><span style={{font
      {unread>0&&<div style={{position:"absolute",top:-4,right:-4,wi
    </div>;})}
  </div>
  ...
  <div style={{display:"flex",gap:5,marginTop:8}}>
    <input value={demoMsg} onChange={e=>setDemoMsg(e.target.value)}
    <div onClick={()=>{if(demoMsg.trim()){const h=new Date().getHour
  </div>
</div>}
{/* SALAIRE */}
{demoPage==="facturation"&&<div style={{padding:10}}>
  ...
  <div style={{display:"flex",gap:4,marginBottom:10}}>
    {["Janv.","Fev.","Mars"].map((m,i)=><div key={m} onClick={()=>se
  </div>
  {[{l:"Salaire de base",d:[{h:"160h x 4,05EUR",v:"648,00 EUR"},{h:"
...
152h x iv styl
          ...
          <div style={{marginTop:8,padding:10,background:"linear-gradient(13
            ...
            <span style={{fontSize:17,fontWeight:700,color:"#C4714A"}}>{["76
          </div>
          <div style={{display:"flex",gap:5,marginTop:8}}>
            <div style={{flex:1,background:"linear-gradient(135deg,#264653,#
            <div style={{flex:1,background:"linear-gradient(135deg,#C4714A,#
          </div>
        </div>}
        {/* OUTILS grise */}
        {demoPage==="outils"&&<div style={{padding:20,display:"flex",flexDir
<divstyle={{fontSize:32}}> </div>
<div style={{fontSize:12,fontWeight:700,color:"#264653"}}>Outils P <div style={{fontSize:10,color:"#9B6BAA",lineHeight:1.6}}>Projet d <div onClick={()=>setShowModal(true)} style={{background:"linear-g
        </div>}
      </div>
      {/* Bottom Nav */}
      <div style={{ display: "flex", justifyContent: "space-around", padding
{[{g:"accueil",ic:" ",l:"Accueil"},{g:"enfant",ic:" ",l:"Enfant"}, <div style={{fontSize:17}} dangerouslySetInnerHTML={{__html:t.ic}} <div style={{fontSize:7,fontWeight:demoGroup===t.g?700:400,color:d
        </div>)}
      </div>
    </div>
    {/* Home indicator */}
...
{/* Legende */}
<div style={{display:"flex",gap:20,justifyContent:"center",flexWrap:"wrap",m
  <div style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#
    <div style={{width:10,height:10,borderRadius:3,background:"linear-gradie
...
<div className="lp-section" style={{ background: "#FDFBF8" }}>
  <div style={{ maxWidth: 900, margin: "0 auto" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax
{[
{id:"mensualisation",cat:"Administratif",catColor:"#FF9F63",emoji:" ",t {id:"maladies",cat:"Santé",catColor:"#E76F51",emoji:" ",title:"Les5ma {id:"agrement",cat:"PMI&Agrément",catColor:"#2A9D8F",emoji:" ",title: {id:"attachement",cat:"Pédagogie",catColor:"#264653",emoji:" ",title:"L {id:"pajemploi",cat:"Administratif",catColor:"#FF9F63",emoji:" ",title: {id:"bulletin",cat:"Administratif",catColor:"#FF9F63",emoji:" ",title:" {id:"trousse",cat:"Santé",catColor:"#E76F51",emoji:" ",title:"Lesindis {id:"tarif",cat:"Administratif",catColor:"#FF9F63",emoji:" ",title:"Com {id:"motricite",cat:"Pédagogie",catColor:"#264653",emoji:" ",title:"Les {id:"droits",cat:"Juridique",catColor:"#2A9D8F",emoji:" ",title:"Droits
         avoir a
 spécif
sont hé
emploi.
...
{showBlog&&<div onClick={e=>e.target===e.currentTarget&&setShowBlog(null)} style
  <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:700,maxHe
   ...
   <div style={{background:"#F0FAF4",borderRadius:12,padding:16,margin:"16p
"Pajem
Compre
e seco
ment f
...
n
  ...
  <div style={{fontWeight:700,color:"#2A9D8F",marginBottom:6}}> Exempl <div style={{fontSize:12}}>
...
{showBlog==="pajemploi"&&<div>
 ...
 <div style={{background:"#FFF8F3",borderRadius:12,padding:16,margin:"16p
x 0",bo force
ttachem
rong>,
> Pa · 7 mi
...
j
  ...
  <div style={{fontWeight:700,color:"#FF9F63",marginBottom:6}}> TiMat
    <div style={{fontSize:12}}>TiMat génère chaque mois un récapitulatif p
  </div>
</div>}
...
<div style={{fontWeight:700,color:"#2A9D8F",marginBottom:6}}> Motric
  ...
  {showBlog==="droits"&&<div>
    ...
    <div style={{background:"#F4F7FA",borderRadius:12,padding:16,margin:"20p
<div style={{fontWeight:700,color:"#264653",marginBottom:8}}> Docume
      <div style={{fontSize:12}}>Contrat signé, avenants, bulletins de salai
    </div>
  </div>}
</div>
  > Le min de repère
...
'<div><h3>Assistante maternelle agréée</h3>', '<strong>'+(user?.prenom||'Prénom')+' '+(user?.nom||'Nom')+'</strong><br/>', 'Email : '+(user?.email||'[email]')+'<br/>',
        ...
        '<strong>'+(enfant?.prenomParent||'Parent')+' '+(enfant?.nomParent||'')+'</str
...
return <div className="fi">
{toast&&<Toast msg={toast}onClose={()=>setToast("")}/>}
<PageHeadericon=" "title="Attestationfiscale"sub="Pourlecréditd'impôtdes {role==="asmat"&&<div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"
    {liste.map(e=><CPill key={e.id}e={e}sel={selId===e.id}onClick={()=>setSelId(e.id
  </div>}
  <div className="g2">
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
...
annee<
    ...
    "</body></html>"
].join(""); w.document.write(html);w.document.close(); setToast("Fiche generee ✓");
};
...
useEffect(()=>{
  if(!user?.id)return;
  supabase.from('invitations').select('*').eq('asmat_id',user.id).order('created_at'
...
setSent(true);
setToast("Invitation envoyée à "+email+" ✓"); setInvitations(prev=>[{id:"inv"+Date.now(),email_parent:email,prenom_enfant:en setEmail("");
    }else{
...
return <div className="fi">
{toast&&<Toast msg={toast} onClose={()=>setToast("")}/>}
<PageHeadericon=" "title="Inviterunparent"sub="Leparentreçoitunemailet
  {/* Sélecteur d'enfant */}
  {enfants.length>1&&<div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wra
    {enfants.map(e=><CPill key={e.id} e={e} sel={selId===e.id} onClick={()=>setSelId
  </div>}
 <div className="g2">
,{ascen
fant.pr
 ...
 <div style={{display:"flex",flexDirection:"column",gap:14}}>
  ...
  <div className="card" style={{padding:20}}>
    <div style={{fontWeight:700,fontSize:14,color:"var(--b)",marginBottom:6}}>
         Inviter le parent de {enfant?.prenom}
    </div>
    <div style={{fontSize:12,color:"var(--l)",marginBottom:16,lineHeight:1.6}}>
      Le parent recevra un email avec un lien pour créer son compte et accéder a
    </div>
    <div style={{marginBottom:12}}>
    ...
    {err&&<div style={{color:"var(--R)",fontSize:12,marginBottom:10,padding:"8px
    {sent&&<div style={{color:"var(--S)",fontSize:12,marginBottom:10,padding:"8p
         Invitation envoyée ! Le parent a reçu un email.
    </div>}
<button className="btn bT" style={{width:"100%",justifyContent:"center"}} onClick={envoyer} disabled={sending||!email.trim()}>
{sending?" Envoi en cours...":" Envoyer l'invitation"}
    </button>
  </div>
  {/* Lien direct (backup) */}
  <div className="card" style={{padding:16,background:"var(--Bp)",border:"1px so
    <div style={{fontWeight:700,fontSize:13,color:"var(--B)",marginBottom:8}}>
    <div style={{fontSize:11,color:"var(--m)",marginBottom:10,lineHeight:1.6}}>
      Envoyez ce lien par SMS, WhatsApp ou autre. Le parent n'a qu'à créer son c
    </div>
    <div style={{display:"flex",gap:8}}>
      <div style={{flex:1,padding:"8px 10px",background:"var(--w)",borderRadius:
        {window.location.origin}/?invite={user?.id?.slice(0,8)}
      </div>
      <button className="btn bG" style={{fontSize:11,padding:"6px 10px",flexShri
        onClick={()=>{navigator.clipboard?.writeText(window.location.origin+"/?i
           Copier
      </button>
    </div>
  </div>
...
<divstyle={{fontSize:32,marginBottom:8}}> </div>
  ...
  :<div style={{display:"flex",flexDirection:"column",gap:8}}>
    {invitations.map((inv,i)=><div key={inv.id||i} style={{display:"flex",just
      <div>
        <div style={{fontSize:13,fontWeight:600,color:"var(--b)"}}>{inv.email_
        <div style={{fontSize:11,color:"var(--l)"}}>
          Pour {inv.prenom_enfant||enfant?.prenom} · {new Date(inv.created_at)
        </div>
      </div>
      <span className="badge" style={{
...
btn:"Je commence →",
       ...
       emoji:" ",
color:"#C4714A", bg:"linear-gradient(135deg,#FEF6F0,#FFF8F4)", titre:"Le journal du quotidien",
ier vo
ius:10,
outiqu
))",gap
...
L
  ...
  {id:"sections",l:"Sections",ic:" "}, {id:"textes",l:"Textes",ic:" "}, {id:"couleurs",l:"Couleurs",ic:" "}, {id:"boutons",l:"Boutons",ic:" "}, {id:"polices",l:"Polices",ic:"𝐓"}, {id:"contenu",l:"Contenu",ic:" "}, {id:"app",l:"App",ic:" "},
];
...
{key:"s5",titre:"Section5-Témoignages",icon:" ",fields:[
  {k:"s5Align",l:"Alignement du texte",type:"align"},
  {k:"s5Title",l:"Titre",type:"txt"},
  {k:"section5Bg",l:"Fond section",type:"col"},{k:"s5TitleColor",l:"Couleu
  {k:"testimonialBg",l:"Fond cards témoignages",type:"col"},{k:"testimonia
  {k:"testimonialCityColor",l:"Couleur ville",type:"col"},{k:"testimonialB
  {k:"testimonialAfterColor",l:"Couleur citation \"après\"",type:"col"},{k
]},
...
r",l:"C
  ...
  {key:"cta",titre:"CTAFinal",icon:" ",fields:[ {k:"ctaAlign",l:"Alignement du texte",type:"align"}, {k:"ctaTitle",l:"Titre (avec \\n)",type:"txt",multi:true},{k:"ctaTitleAc {k:"ctaSub",l:"Texte descriptif",type:"txt",inTxts:true},{k:"ctaBtnTxt", {k:"ctaBg",l:"Fond section",type:"col"},{k:"ctaTitleColor",l:"Couleur ti {k:"ctaSubTitleColor",l:"Couleur sous-titre",type:"col"},{k:"ctaSubColor
    ]},
...
transformations:[
[" ","Pajemploivousprend2hparmois","Récapprêten5minutes","Zéroerreur.Z [" ","Voscontratssontdansuntiroir","Modèlesguidés,avenantsen2clics","So [" ","Lesretardsdeparentscréentdesconflits","Pointagehorodaté,signéparl [" ","Undocumentimportantestintrouvable","Toutcentralisé,daté,cherchable",
          0&famil
t\ntout
 jamais
isatio
...
r u x s
  ...
  [" ","Vossoiréesserventàl'administratif","5minuteslematinsuffisent","Vos ],
statsHero:[
...
// Utiliser données réelles si disponibles, sinon démo
 ...
 const enfants=dbLoading&&!isDemo?[]:(hasRealData?enfantsDB:(isDemo||role==="asmat"?D
