...
@media(max-width:768px){.toast{bottom:80px;font-size:12px;padding:10px 16px;border-radius
...
return <div className="fi">
{toast&&<Toast msg={toast}onClose={()=>setToast("")}/>}
<PageHeader icon=" " title="Repas & Changes" sub="Suivi alimentaire et hygiène du {role==="asmat"&&<div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
jour"/
{liste.map(e=><CPill key={e.id}e={e}sel={selId===e.id}onClick={()=>setSelId(e.id)}/>)}<
<div className="g2">
<div className="card"style={{padding:16}}>
...
<div style={{background:"var(--Bp)",borderRadius:10,padding:"10px 14px",marginBottom:
Votre assmat sera notifiée immédiatement. L'absence sera notée dans votre </div>
calend
<div style={{display:"grid",gap:12}}>
...
<div style={{fontWeight:700,marginBottom:4}}> {enfant.prenom} {enfant.nom} - {age
<div style={{fontSize:11,color:"#666"}}>Période d'accueil : {enfant.contrat?.horair
</div>
<table>
...
solid
{/* Formulaire ajout */}
{showUpload&&<div className="card"style={{padding:18,marginBottom:16,border:"1.5px <div style={{fontWeight:700,fontSize:13,marginBottom:12,color:"var(--b)"}}> Ajouter u
<div className="g2"style={{marginBottom:10}}>
<div>
...
<div style={{ width: 340, flexShrink: 0, background: "#1a1a2e", borderRadius: 44,
{/* Notch */}
<div style={{ display: "flex", justifyContent: "center", marginBottom: 8 }}>
<div style={{ width: 110, height: 24, background: "#1a1a2e", borderRadius: "0
<div style={{ width: 7, height: 7, borderRadius: "50%", background: "#2a2a4
<div style={{ width: 48, height: 4, borderRadius: 2, background: "#2a2a4e"
</div>
</div>
{/* Screen */}
<div style={{ background: "#FDFBF8", borderRadius: 30, overflow: "hidden", heig
{/* Badge DEMO */}
<div style={{position:"absolute",top:8,right:8,zIndex:10,background:"rgba(155
{/* TopBar */}
<div style={{ background: "rgba(255,255,255,.98)", borderBottom: "1px solid r
<div style={{ display: "flex", alignItems: "center", gap: 6 }}>
...
<div style={{position:"relative"}}>
<span style={{ fontSize: 14 }}> </span>
<div style={{position:"absolute",top:-3,right:-3,width:8,height:8,borde
</div>
<span style={{ fontSize: 14 }}> </span>
...
{demoGroup!=="accueil"&&<div style={{ background: "#fff", borderBottom: "1px
{(demoGroup==="enfant"?[
{id:"journal",l:" Journal",ok:true},{id:"pointage",l:" Pointage",ok:t
{id:"sante",l:" Santé",ok:false},{id:"urgence",l:" Urgence",ok:false
]:demoGroup==="admin"?[
{id:"messagerie",l:" Messages",ok:true},{id:"facturation",l:" Salaire
{id:"calendrier",l:" Calendrier",ok:false},{id:"documents",l:" Docume
]:[
{id:"projet",l:" Projet accueil",ok:false},{id:"pmi",l:" PMI",ok:fals
]).map(s=><div key={s.id} onClick={s.ok?()=>setDemoPage(s.id):undefined} st
whiteSpace:"nowrap",padding:"4px 10px",borderRadius:8,fontSize:9,fontWeig
background:demoPage===s.id?"linear-gradient(135deg,#C4714A,#D4824A)":s.ok
...
return <div key={e.id} onClick={()=>setDemoArrivee(prev=>({...prev,[e.i
<span style={{fontSize:20}}>{e.emoji}</span>
<div style={{flex:1}}>
<div style={{fontSize:11,fontWeight:700,color:"#264653"}}>{e.prenom
<div style={{fontSize:9,color:"#9B6BAA"}}>{estPresent?`Arrivé à ${d
</div>
<div style={{fontSize:8,padding:"2px 7px",borderRadius:6,background:e
</div>;
})}
...
<div style={{fontSize:12,fontWeight:700,color:"#264653"}}> Journal —
<div style={{display:"flex",gap:4}}>
{D.enfants.map(e=><div key={e.id}style={{width:24,height:24,borderRad
</div>
</div>
{D.transmissions.filter(t=>t.eId==="e1").map(t=><div key={t.id}style={{ba
...
<button onClick={()=>setDemoLiked(v=>!v)} style={{background:"none",b
</div>
</div>)}
<div style={{display:"flex",gap:4,marginBottom:6}}>
{[" "," "," "," "," "," "].map(m=><div key={m} onClick={()=>setD
</div>
<div style={{display:"flex",gap:5}}>
<input value={demoMsg} onChange={e=>setDemoMsg(e.target.value)} placeho
<div onClick={()=>{if(demoMsg.trim())setDemoMsg("");}} style={{backgrou
</div>
</div>}
{/* POINTAGE */}
...
<div style={{flex:1}}>
<div style={{fontSize:10,fontWeight:700,color:"#264653"}}>{e.prenom
<div style={{fontSize:9,color:"#9B6BAA"}}>{estPresent?`Arrivé ${dem
</div>
<div style={{fontSize:9,fontWeight:700,background:estPresent?"#F0FAF4
</div>;
})}
</div>
</div>}
<div style={{background:"linear-gradient(135deg,#C4714A,#D4824A)",borderR
<span style={{color:"#fff",fontSize:10,fontWeight:700}}> QR Code pare
{/* MESSAGERIE */}
{demoPage==="messagerie"&&<div style={{padding:10,display:"flex",flexDirect
...
{demoMsgs.filter(m=>m.eId==="e1").map(m=><div key={m.id}style={{alignSe
...
<input value={demoMsg} onChange={e=>setDemoMsg(e.target.value)} onKeyDo
<div onClick={()=>{if(demoMsg.trim()){const h=new Date().getHours()+"h"
</div>
</div>}
{/* FACTURATION / SALAIRE */}
...
{["Janv.","Fév.","Mars"].map((m,i)=><div key={m} onClick={()=>setDemoMo
</div>
{[
brut</
{l:"Salaire de base",d:[{h:"160h × 4,05€",v:"648,00 €"},{h:"152h × 4,05
{l:"Ind. entretien",d:[{h:"20j × 3,80€",v:"76,00 €"},{h:"19j × 3,80€",v
{l:"Heures majorées",d:[{h:"8h × 5,06€",v:"40,50 €"},{h:"4h × 5,06€",v:
...
<span style={{fontSize:11,fontWeight:700,color:"#264653"}}>Total <span style={{fontSize:17,fontWeight:700,color:"#C4714A"}}>{["764,50 €"
</div>
<div style={{display:"flex",gap:5,marginTop:8}}>
...
{/* OUTILS - tout grisé */}
{demoPage==="outils"&&<div style={{padding:20,display:"flex",flexDirection:
<div style={{fontSize:32}}> </div>
<div style={{fontSize:12,fontWeight:700,color:"#264653",textAlign:"center
<div style={{fontSize:10,color:"#9B6BAA",textAlign:"center",lineHeight:1.
<div style={{background:"linear-gradient(135deg,#9B6BAA,#B87CC8)",borderR
</div>}
</div>
...
].map(t=><div key={t.g} onClick={()=>{setDemoGroup(t.g);if(t.g==="accueil")
<div style={{fontSize:17}}>{t.ic}</div>
<div style={{fontSize:7,fontWeight:demoGroup===t.g?700:400,color:demoGrou
</div>)}
</div>
</div>
{/* Home indicator */}
...
<div style={{display:"flex",gap:20,justifyContent:"center",flexWrap:"wrap",marginTo
<div style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#6B4F3A"
<div style={{width:10,height:10,borderRadius:3,background:"linear-gradient(135d
...
<div style={{marginTop:16,textAlign:"center",fontSize:11,color:"#B0BEC5"}}> </div>
</div>}
Paiem
{/* FOOTER */}
...
Cette attestation permet aux parents de bénéficier du crédit d'impôt pour </div>
</div>
<div className="card"style={{padding:18}}>
...
<div style={{display:"flex",flexDirection:"column",gap:12}}>
<div className="card"style={{padding:16}}>
<div style={{fontWeight:700,fontSize:13,color:"var(--b)",marginBottom:12}}> {inp("Nom","nom")}{inp("Prenom","prenom")}{inp("Date de naissance","naissance","JJ/
</div>
<div className="card"style={{padding:16}}>
...
<div style={{display:"flex",flexDirection:"column",gap:12}}>
<div className="card"style={{padding:16}}>
<div style={{fontWeight:700,fontSize:13,color:"var(--b)",marginBottom:12}}> {inp("Nom et prenom","nom")}{inp("Adresse","adresse")}{inp("Telephone","tel")}{inp(
</div>
<div className="card"style={{padding:16}}>
...
<div className="card"style={{padding:16}}>
<div style={{fontWeight:700,fontSize:13,color:"var(--b)",marginBottom:12}}> Mes s
