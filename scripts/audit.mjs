...
// configuration qui l'ecrasait — donc rien ne plantait, et la landing partait
// en ligne avec deux titres illisibles.
//
// On rapproche chaque couleur de texte du fond de SA section, tel que
// DEFAULT_CONFIG les declare.
{
  const app = readFileSync(fichiersAppSrc().find((u) => u.pathname.endsWith("/App.jsx")), "utf8");
  const val = (cle) => (app.match(new RegExp(cle + ':\\s*"([^"]+)"')) || [])[1];
  // Un degrade : on prend sa couleur la plus claire ET la plus sombre, et on
  // exige que le texte passe sur les deux — un titre lisible en haut du degrade
  // et noye en bas reste un titre illisible.
  ...
  for (const [cTexte, cFond, seuil] of COUPLES) {
    ...
    // Une couleur translucide se melange a ce qu'il y a dessous : on ne peut pas
    // trancher sans rendre la page, et inventer une reponse serait pire.
